...
function frameworkScripts(framework: ProjectManifestOptions["framework"]): Record<string, string> {
  if (framework === "astro") {
    return {
      dev: "astro dev",
      build: "astro build",
...
function frameworkDependencies(framework: ProjectManifestOptions["framework"]): Record<string, string> {
  if (framework === "astro") {
    return {
...
function frameworkDevDependencies(framework: ProjectManifestOptions["framework"]): Record<string, string> {
  if (framework === "astro") {
    return {
      typescript: "^5.6.2",
    };
...
export function buildProjectManifest(options: ProjectManifestOptions): Record<string, unknown> {
...
export function renderCmsConfig(
  presetId: ProjectManifestOptions["preset"],
  pluginPackages?: string[],
  framework: ProjectManifestOptions["framework"] = "nextjs"
): string {
