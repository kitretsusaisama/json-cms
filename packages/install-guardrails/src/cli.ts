...
function parseOptions(argv: readonly string[]): ParsedOptions {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token.startsWith("--framework=")) {
      ...
      if (!value) out.errors.push("Missing value for --framework.");
      else out.framework = value;
      continue;
    }
    ...
    if (token.startsWith("--adapter=")) {
      ...
      if (!value) out.errors.push("Missing value for --adapter.");
      else out.adapter = value;
      continue;
    }
...
const report = runInstallGuardrails({
