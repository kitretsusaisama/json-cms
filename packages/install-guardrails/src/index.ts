...
type JsonReadResult =
  ...
  | { ok: false; reason: "missing" | "invalid" | "unreadable"; error?: Error };

type PackageVersionResult =
  | { version: string; packageJsonPath: string }
  | { version: null; reason: "missing" | "invalid" | "unreadable"; packageJsonPath?: string; error?: Error };
...
function readJson(filePath: string): JsonReadResult {
  try {
    return {
      ok: true,
      value: JSON.parse(readFileSync(filePath, "utf8")) as Record<string, unknown>,
    };
  } catch (error) {
    const err = error as NodeJS.ErrnoException;
    if (err.code === "ENOENT") return { ok: false, reason: "missing" };
    if (error instanceof SyntaxError) return { ok: false, reason: "invalid", error };
    return { ok: false, reason: "unreadable", error: error as Error };
  }
}

function detectFramework(pkg: ProjectPackageJson): string {
  const deps = {
    ...pkg.dependencies,
    ...pkg.devDependencies,
  };
...
function packageVersion(cwd: string, packageName: string): PackageVersionResult {
...
function collectPluginNames(pkg: ProjectPackageJson): string[] {
  const names = new Set<string>();
  for (const group of [pkg.dependencies, pkg.optionalDependencies]) {
    for (const name of Object.keys(group ?? {})) {
      if (name.startsWith("@upflame/plugin-")) names.add(name);
    }
  }
  return [...names].sort();
}

function readPluginCompatibility(cwd: string, pluginName: string): {
  ...
  const pluginDir = path.dirname(packageResult.packageJsonPath);
  ...
  const fromManifest = manifestValue?.compatibility as Record<string, unknown> | undefined;
  const engines = manifestValue?.engines as Record<string, unknown> | undefined;
  const peerDeps = pkgRead.value.peerDependencies as Record<string, string> | undefined;
  ...
  const jsonCms =
    (typeof fromManifest?.jsonCms === "string" && fromManifest.jsonCms) ||
...
function isSupportedRange(range: string): boolean {
  return semver.validRange(range, { includePrerelease: true }) !== null;
}

export function runInstallGuardrails(options: GuardrailOptions = {}): GuardrailReport {
  ...
  const projectPkg = projectPkgRead.value as ProjectPackageJson;
  ...
  for (const pluginName of collectPluginNames(projectPkg)) {
    ...
    if (!pluginVersionResult.version) continue;
    ...
    if (installedCore && compatibility.jsonCms) {
      ...
      } else if (!semver.satisfies(installedCore, compatibility.jsonCms, { includePrerelease: true })) {
        diagnostics.push({
          level: "error",
    ...
    for (const [adapterName, range] of Object.entries(compatibility.adapters ?? {})) {
      ...
      if (!semver.satisfies(installedAdapter.version, range, { includePrerelease: true })) {
        diagnostics.push({
          level: "error",
