...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    const health = await pluginHost.getPluginHealth(pluginId);
    return ok(request, { pluginId, health });
  } catch (error) {
    return internalServerError(`Failed to load health for plugin \"${pluginId}\".`, error);
  }
}
