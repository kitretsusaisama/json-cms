...
exports.useCMSOptional = useCMSOptional;
...
// ─── Context ──────────────────────────────────────────────────────────────────
const CMSContext = (0, react_1.createContext)(null);
function useCMS() {
    const ctx = (0, react_1.useContext)(CMSContext);
    if (!ctx)
        throw new Error("useCMS must be used inside <CMSProvider>");
    return ctx;
}
function useCMSOptional() {
    return (0, react_1.useContext)(CMSContext);
}
...
// ─── Provider ─────────────────────────────────────────────────────────────────
function CMSProvider({ children, isPreview = false, isDebug: initialDebug = false, locale = "en", abBucket = 0, nonce, analyticsEndpoint = "/api/cms/analytics", enableAutoTracking = true, }) {
    const [isDebug, setIsDebug] = (0, react_1.useState)(initialDebug || (typeof window !== "undefined" && new URLSearchParams(window.location.search).has("cms_debug")));
    const statsRef = (0, react_1.useRef)(new Map());
    ...
    return ((0, jsx_runtime_1.jsx)(CMSContext.Provider, { value: {
