...
type Command = "init" | "add" | "add-plugin";

function normalizePluginFlagValue(value: string): string[] {
  return value
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean)
...
function parseArgs(argv: string[]): Partial<CreateJsonCmsOptions> {
  const options: Partial<CreateJsonCmsOptions> = {};
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (value) {
      case "--dir":
        ...
        break;
      case "--framework":
        options.framework = argv[index + 1] as CreateJsonCmsOptions["framework"];
        index += 1;
        break;
      case "--preset":
        options.preset = argv[index + 1] as CreateJsonCmsOptions["preset"];
        index += 1;
        break;
      case "--mode":
        options.preset = argv[index + 1] as CreateJsonCmsOptions["preset"];
      ...
      case "--package-manager":
        options.packageManager = argv[index + 1] as CreateJsonCmsOptions["packageManager"];
        index += 1;
        break;
      case "--no-install":
...
function resolveCommand(argv: string[]): { command: Command; rest: string[] } {
  const first = argv[0];
...
async function resolveFramework(parsed: Partial<CreateJsonCmsOptions>): Promise<CreateJsonCmsOptions["framework"]> {
  if (parsed.framework) {
  ...
  if (detection.detected && detection.supported) {
    return detection.detected as CreateJsonCmsOptions["framework"];
  }
  if (detection.detected && !detection.supported && (detection.confidence ?? 0) >= 0.4) {
    ...
    console.warn(
      `[create-json-cms] Detected ${detection.detected}${confidence}, but this framework is not yet scaffold-ready. Falling back to --framework next.`
    );
  }
  return "nextjs";
}

async function resolveOptions(argv: string[], nonInteractive: boolean, command: Command): Promise<CreateJsonCmsOptions> {
...
export async function runCreateJsonCms(argv = process.argv.slice(2)): Promise<void> {
