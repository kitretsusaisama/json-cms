...
function resolveTemplateDirectoryForFramework(framework: CreateJsonCmsOptions["framework"]): string {
...
function mergeDependencies(current: Record<string, string> | undefined, ordered: string[]): Record<string, string> {
...
function frameworkRoutePath(framework: CreateJsonCmsOptions["framework"]): string {
  if (framework === "astro") {
...
function frameworkRouteTemplate(framework: CreateJsonCmsOptions["framework"]): string {
  if (framework === "astro") {
    return `---
---
<html lang="en">
...
export async function addIntegrationToProject(options: CreateJsonCmsOptions): Promise<{ projectDir: string }> {
  ...
  const currentPackageJson = JSON.parse(await readFile(packageJsonPath, "utf-8")) as {
    name?: string;
  ...
  const nextPackageJson = {
    ...
    devDependencies: {
      ...
      ...Object.fromEntries(
        Object.entries(manifest.devDependencies).filter(([name]) => !(currentPackageJson.devDependencies ?? {})[name])
      ),
    },
  };
